...
export const placeOrder = async () => {
    try {
        ...
        if (successfulOrders.length === cart.length) {
            ...
            console.warn('Some items failed to process. Shopping cart will not be cleared.', results.filter((result) => !result.success));
        }
...
export const getUserOrders = async (userId) => {
    try {
        const response = await fetch(`http://35.183.86.101/order/user/${encodeURIComponent(userId)}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
            },
