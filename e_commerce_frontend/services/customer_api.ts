...
export const customerLogin = async (username: string, password: string): Promise<any | null> => {
  try {
...
export const registerCustomer = async (registerRequest: any): Promise<boolean> => {
  try {
...
export const getCustomerAddress = async (addressID: number): Promise<boolean | null> => {
  try {
...
export const getCustomerBillingInfo = async (id: number): Promise<boolean | null> => {
  try {
...
export const updateCustomer = async (updatedCustomer: User): Promise<boolean> => {
  try {
...
export const updateAddress = async (updatedAddress: Address): Promise<boolean> => {
  try {
    const response = await fetch(`${ADDRESS_API_URL}`, { // Use POST and match the backend URL
      method: 'POST', // Change to POST since the backend uses @PostMapping("")
      headers: {
        'Content-Type': 'application/json',
      },
...
export const updateBillingInfo = async (updatedBilling: Billing): Promise<boolean> => {
  try {
    const response = await fetch(`${BILLING_API_URL}/update`, {  // Ensure this matches the backend URL
...
export const updateAllInfo = async (): Promise<boolean> => {
  try {
    ...
    const parsedAddressData: Address = JSON.parse(addressData);
    const parsedBillingData: Billing = JSON.parse(billingData);
